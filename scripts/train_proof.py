"""Phase 2: Train proof decoder (FOL representations → proof chain).

Translation decoder is frozen. Proof decoder cross-attends to translation
decoder hidden states — structured FOL only, no NL in its context.

Proof decoder input is [<extra_id_3>, proof_step_1, ...] truncated at <extra_id_4>.
It never sees the answer token.
...
def evaluate(model, val_loader, device, use_bf16):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            trans_pad_mask = batch["trans_padding_mask"].to(device)
            ...
            with autocast("cuda", dtype=dtype, enabled=use_bf16):
                with torch.no_grad():
                    _, trans_hidden, _ = model.forward_translation(input_ids, attn_mask, trans_input)
                proof_logits = model.forward_proof(trans_hidden, proof_input, trans_pad_mask)
...
def main(config_path: str = "configs/v12_proof.yaml"):
    ...
    # Load model and translation checkpoint
    ...
    # Curriculum
    first_qdep = phases[0].get("max_qdep") if phases else None
    train_loader = make_loader(max_qdep=first_qdep)
    curriculum = []
    for i, phase in enumerate(phases[1:], 1):
        switch_at = phases[i - 1]["until_step"]
        curriculum.append((switch_at, make_loader(max_qdep=phase.get("max_qdep"))))
        print(f"Curriculum phase {i+1}: starts at step {switch_at}, max_qdep={phase.get('max_qdep')}")
    ...
    for step in range(tcfg["max_steps"]):
        if curriculum and step >= curriculum[0][0]:
            ...
            data_iter = iter(train_loader)
            print(f"[step {step}] Curriculum: switching to next phase loader")
        ...
        dtype = torch.bfloat16 if use_bf16 else torch.float32
        with autocast("cuda", dtype=dtype, enabled=use_bf16):
            # Translation decoder is frozen — no_grad for efficiency
            with torch.no_grad():
                _, trans_hidden, _ = model.forward_translation(input_ids, attn_mask, trans_input)
            proof_logits = model.forward_proof(trans_hidden, proof_input, trans_pad_mask)
            B, T, V = proof_logits.shape
